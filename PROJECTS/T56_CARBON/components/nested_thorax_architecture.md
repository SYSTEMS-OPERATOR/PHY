# T-5.6 Nested Thorax Architecture

Status: `CONCEPT_SCAFFOLD_NOT_FABRICATION_READY`

This document defines the nested thorax architecture for the T-5.6 domestic humanoid armature.

> Primary structure within. Secondary form without. Softness above structure.
...
The thorax must preserve an adult female domestic profile without an armored barrel chest or exaggerated display anatomy. Required qualities include an elegant clavicle transition, restrained chest projection, underbust taper, lateral rib softness, believable waist transition, and continuity into the abdomen and pelvis.

## Service doctrine

The module must support independent access to:
...
- Carbon conductivity is modeled explicitly.
- Direct carbon-to-aluminum contact is prohibited without a validated dielectric barrier.
- Power, grounding, tactile sensing, and structural-health sensing must use intentional electrical zones.
- Heat-producing components reject heat through the inner housing, not into the contour padding.

## Release blockers

- validated T-5.6 thoracic anthropometry
- primary load cases and safety factors
- selected metal chassis material and insert system
...
> Metal heart. Isolated breath. Carbon ribs. Soft contour.
